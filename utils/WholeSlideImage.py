...
import h5py
from wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag
...
class WholeSlideImage(object):
    def __init__(self, path, hdf5_file=None):
        #self.name = ".".join(path.split("/")[-1].split('.')[:-1]) # for linux
        #self.name = ".".join(path.split("\\")[-1].split('.')[:-1]) # for windows
    ...
    def makemask(self, seg_level, xml_path):
        """
        ...
        self.holes_tissue = self.scaleHolesDim([[]], scale)
    ...
    def createPatches_bag_hdf5(self, patch_save_path, patch_level=0, patch_size=256, step_size=256, save_coord=True, **kwargs):
        contours = self.contours_tissue
        print("Creating patches for: ", self.name, "...",)
        for idx, cont in enumerate(contours):
            patch_gen = self._getPatchGenerator(cont, idx, patch_level, patch_save_path, patch_size, step_size, **kwargs)
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, patch_save_path, patch_size=256, step_size=256, custom_downsample=1,
        white_black=True, white_thresh=15, black_thresh=50, contour_fn='four_pt', remove_around_black=True, 
        circle=1, use_padding=True):
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                if self.holes_tissue == [[]]:
                    if not self.isInContours(cont_check_fn, cont, (x,y), None, ref_patch_size[0]): #point not inside contour and its associated holes
                        continue
                else:
                ...
                if white_black:
                    if self.isBlackPatch(np.array(patch_PIL), rgbThresh=black_thresh):
                        position = (x,y)
                        bound = [start_x,start_y,stop_x,stop_y]
                        skip_list = self.get_around_positions(position,step_size_x,step_size_y, bound, circle)
                        black_list += skip_list
                        black_list = list(set(black_list))
                        continue
                    if self.isWhitePatch(np.array(patch_PIL), satThresh=white_thresh): 
                        continue
                if remove_around_black:
                    if (x,y) in black_list:
                        continue
                ...
                'patch_PIL':patch_PIL, 'name':self.name, 'patch_save_path':patch_save_path}
    ...
    @staticmethod
    def get_around_positions(position,step_x,step_y, bound, circle):
        skip_list = []
        n = circle * 2 + 1
        x = position[0] - circle * step_x
        y = position[1] - circle * step_y
        for i in range(n):
            for j in range(n):
                x_tmp = x+j*step_x
                y_tmp = y+i*step_y
                if x_tmp >= bound[0] and x_tmp<= bound[2] and y_tmp >= bound[1] and y_tmp <= bound[3]:
                    skip_list.append((x_tmp,y_tmp))
        return skip_list
